    ...
    <section id="systems" className="py-24 relative bg-slate-950/90 border-t border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-20">
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <h3 className="text-lg font-bold text-slate-100 flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-sky-400" /> Six Core Areas of Technology Management
            </h3>
            <span className="text-xs font-mono text-slate-400">Professional Methodology</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {corePillars.map((p) => {
              ...
              return (
                <div
                  key={p.num}
                  className="glass-panel p-6 rounded-2xl border border-slate-800 hover:border-sky-500/40 transition-all duration-300 space-y-4 group"
                >
                  <div className="flex items-center justify-between">
                    <div
                      className={`w-10 h-10 rounded-xl bg-gradient-to-br ${p.color} flex items-center justify-center text-white font-bold shadow-md`}
                    >
                      <IconComp className="w-5 h-5" />
                    </div>
                    <span className="text-xl font-mono font-extrabold text-slate-700 group-hover:text-sky-400 transition-colors">
                      {p.num}
                    </span>
                  </div>

                  <div>
                    <h4 className="text-base font-bold text-slate-100 group-hover:text-sky-300 transition-colors mb-1">
                      {p.title}
                    </h4>
                    <p className="text-xs font-semibold text-sky-400 mb-2">{p.desc}</p>
        ...
        <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-slate-800 space-y-8">
            ...
            {LIFECYCLE_STAGES.map((s) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-2 rounded-xl text-xs font-mono font-bold transition-all whitespace-nowrap flex items-center gap-1.5 border ${
                    isActive
                      ? 'bg-sky-500 text-white border-sky-400 shadow-md shadow-sky-500/20'
                      : 'bg-slate-900 text-slate-400 border-slate-800 hover:text-slate-200 hover:bg-slate-800'
                  }`}
                >
                  <span className="opacity-75">{s.step}</span>
                  <span>{s.name}</span>
                </button>
              );
            })}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 rounded-2xl bg-slate-950 border border-sky-500/30">
            <div className="lg:col-span-4 space-y-4">
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-1 rounded text-xs font-mono font-bold bg-sky-500/20 text-sky-400 border border-sky-500/30">
                  STAGE {activeStage.step}
                </span>
                <h4 className="text-lg font-bold text-slate-100">{activeStage.name}</h4>
              </div>
              ...
              <div className="p-3.5 rounded-xl bg-slate-900/80 border border-slate-800 text-xs space-y-1">
                ...
                <p className="text-slate-300 italic">{activeStage.experienceContext}</p>
              </div>
            </div>

            <div className="lg:col-span-4 space-y-3 p-4 rounded-xl bg-slate-900/60 border border-slate-800">
              <h5 className="text-xs font-mono font-bold text-amber-400 uppercase flex items-center gap-2">
                <HelpCircle className="w-4 h-4" /> Key Discovery Questions
              </h5>
            ...
            <div className="lg:col-span-4 space-y-3 p-4 rounded-xl bg-slate-900/60 border border-slate-800">
              <h5 className="text-xs font-mono font-bold text-emerald-400 uppercase flex items-center gap-2">
                <FolderCheck className="w-4 h-4" /> Typical Phase Outputs
              </h5>
              <ul className="space-y-2">
        ...
        <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-slate-800 space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
            <div>
              <div className="text-xs font-mono font-bold text-rose-400 uppercase tracking-wider mb-1 flex items-center gap-2">
                <AlertOctagon className="w-4 h-4" /> GOVERNANCE & CONTROL
              </div>
              <h3 className="text-xl font-extrabold text-slate-100">
            ...
            {/* Workflow Flow pills */}
            <div className="flex flex-wrap items-center gap-1.5 text-[10px] font-mono text-slate-400 bg-slate-950 p-2 rounded-xl border border-slate-800">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                return (
                  <button
                    ...
                    className={`w-full p-3 rounded-xl border text-left text-xs transition-all flex items-center justify-between ${
                      isActive
                        ? 'bg-slate-900 border-rose-500/60 text-white font-bold shadow-md'
                        : 'bg-slate-950/60 border-slate-800 text-slate-400 hover:text-slate-200 hover:bg-slate-900/50'
                    }`}
                  >
                    <span>{r.category}</span>
                    <ChevronRight
                      className={`w-4 h-4 ${isActive ? 'text-rose-400' : 'text-slate-600'}`}
                    />
                  </button>
                );
              })}
            ...
            <div className="lg:col-span-8 p-6 rounded-2xl bg-slate-950 border border-slate-800 space-y-4">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <h4 className="text-base font-bold text-slate-100 flex items-center gap-2">
                  <ShieldAlert className="w-5 h-5 text-rose-400" />
                  {activeRisk.category}
                </h4>
                <span className="px-2.5 py-0.5 rounded text-[10px] font-mono bg-rose-500/10 text-rose-400 border border-rose-500/20 font-bold">
                  Proactive Control
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                ...
                <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800 space-y-3">
                  ...
                  <div className="pt-2 border-t border-slate-800">
                    ...
                    <p className="text-xs text-slate-300">{activeRisk.monitoringApproach}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-slate-800 space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
            <div>
              <div className="text-xs font-mono font-bold text-sky-400 uppercase tracking-wider mb-1 flex items-center gap-2">
                <Users className="w-4 h-4" /> CROSS-FUNCTIONAL ALIGNMENT
              </div>
              <h3 className="text-xl font-extrabold text-slate-100">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                return (
                  <button
                    ...
                    className={`p-3 rounded-xl border text-left text-xs transition-all flex items-center justify-between ${
                      isActive
                        ? 'bg-slate-900 border-sky-500/70 text-white font-bold shadow-md'
                        : 'bg-slate-950/60 border-slate-800 text-slate-400 hover:text-slate-200 hover:bg-slate-900/50'
                    }`}
                  >
                    <span>{sg.role}</span>
            ...
            <div className="lg:col-span-7 p-6 rounded-2xl bg-slate-950 border border-slate-800 space-y-5">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <h4 className="text-base font-bold text-slate-100 flex items-center gap-2">
